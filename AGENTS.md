...
- 资产正文只面向读者。本仓库的内部事务——建设规划（缺什么决策入口、打算补什么条目）、收录与润色过程、对其他资产的处理安排——不写进 `registry/`、`articles/`、`projects/` 的正文；要记录放 `docs/` 或 issue。分界线：引用 DECIDE.md 说明「何时走到这里」是读者信息，可以写；说「本库尚无／需要时补」是维护者备注，不写。
