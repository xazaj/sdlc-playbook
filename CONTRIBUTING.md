...
projects/  项目解剖卡。优秀开源仓库的设计借鉴分析，给人读，不进 agent 上下文
bin/       安装与更新检查脚本
...
| 新增项目卡 | `projects/<owner>-<repo>.md` | 某个开源项目的设计解法值得解剖借鉴 |
...
1. **蒸馏为默认，全译需声明。** 原文不进库（版权边界见文末）；成稿默认是观点重述，可核对的数字、结论与引语保留并标明出处。收录整篇译文时，先确认上游许可允许演绎传播，frontmatter 标 `translated: true` 即完成声明。
2. **带来源与日期。** frontmatter 的 `source` 指回原文，`date` 是蒸馏定稿日，任何时候可回溯核对。
3. **summary 答得出「为什么留着它」。** 答不出就是剪藏，不收。

标题一条规则：原文有标题就译其原题；原文没有标题时，为成稿定一个最切题的标题，不套「完全指南」「一文读懂」一类模板后缀。

正文自由，一条约定：开头一段导语说清文章主张什么。来源不写在正文里，文章页底部自动渲染来源卡（作者、原文链接）；整篇译文在 frontmatter 标 `translated: true`，来源卡加「中文全译」标记。正文不写「来源与边界」一节，也不出现润色过程、收录经过、库内建设规划一类读者不关心的过程性说明。`related` 挂关联的登记卡，文章页右栏互链。页面随首批文章一起构建，不留空归档页。
...
3. 正文开头一段导语说清主张。来源交给页面底部的来源卡，正文不写
   来源节，也不写润色过程、库内建设规划一类过程性说明；译文在 frontmatter 标
   translated: true。工具与佐证链接就地内联。
...
项目卡是第四类资产：对单个开源仓库的解剖，给人读，回答「这个设计问题别人怎么解的、什么能搬」。与文章的分工：文章蒸馏别人的论述，项目卡是本库自己的分析，每条关键断言给锚定 commit 的 permalink，读者一键核对。目录 `projects/<owner>-<repo>.md`，模板是同目录的 `_TEMPLATE.md`（构建排除 `_` 开头文件），排版规范见 DESIGN.md 的「项目」一节。

收录三门槛，缺一不收：

1. **判断而非推荐。** 「借鉴清单」必须分「能搬的」与「不适用的」两半，不适用那半句不许省；只说好话的卡是软文，不收。
2. **快照钉 commit。** frontmatter 的 stars、language、license 是评估当天的事实，`pinned_commit` 是全文证据锚定的那个 commit，不追新、不更新——与登记卡规则 2 同构。
3. **证据可点开。** 正文至少一个 `blob/<commit>/…` permalink；「它解决什么问题」「设计亮点」「借鉴清单」三节缺任一，构建失败并指名文件。

中文成稿按 sepia 技术文章路线润色（professional-pass + tech-articles，中文加 zh 校准），成品不留润色痕迹，正文也不写评估过程与库内建设规划（AGENTS.md 硬性约束同此）。

### 让 agent 解剖项目

复制整块，贴进本仓库的 agent 会话，把方括号换成实际仓库。它描述结果而非命令，可重复执行：

````text
请为本仓库解剖 [GitHub 仓库地址]，产出一张项目卡，要求：

1. 先读 projects/_TEMPLATE.md 与 site/src/content.config.ts 里 projects 的
   schema，以它们为准，不要凭记忆填字段。

2. 把仓库读透：README、目录结构、关键源文件。结论只写你实际读到的
   东西，不转述上游宣传语；读不到的停下来问我，不要猜。

3. 查证快照：stars、language、license，以及你克隆或抓取时 HEAD 的完整
   commit SHA。全文证据链接全部锚定这个 SHA。

4. 复制模板为 projects/<owner>-<repo>.md，按模板小节写正文。「借鉴
   清单」分「能搬的」与「不适用的」两半，不适用那半句不许省。关键
   断言给 blob/<commit>/… 形式的 permalink。

5. 中文正文按 sepia 技术文章路线润色后定稿（先读规范再落笔）：删掉
   连接词堆叠、双音节垫字与三段式排比，句子长短不齐，判断要有立场。
   不写本库的建设规划与收录润色过程，这类内容不进正文。

6. 运行 cd site && npm run build，构建失败就修到通过。

7. 完成后列出改动或新增的文件，说明哪些字段查证过、哪些要我复核。
````

生成之后必须人工过一遍两件事：permalink 是否真的指向你读过的那个 commit，快照数字是否抄对。这两条模型最容易糊弄过去，而它们恰好是项目卡的全部价值所在。
