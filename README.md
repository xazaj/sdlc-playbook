...
开发过程中反复出现一类选型问题：界面该用哪套设计系统，这次改动该先写测试还是直接改。这类问题没有通用答案，取决于当前项目的具体情况。

SDLC Playbook 把这些问题按开发阶段整理成选型矩阵，并安装为 Claude Code 技能。装好之后，在任意项目里提出这类问题，agent 会自动读取对应矩阵，追问几个判断依据，然后给出一个带理由的建议，并指向执行工具。

同一批内容也渲染成站点：<https://aibrev.com/sdlc-playbook/>。不装技能也能直接翻，每个条目附可直接复制的调用 prompt。
...
预期行为：`sdlc-coding-process` 技能被触发，agent 追问改动的验证成本、范围和需求确定性，然后在 TDD、SDD、直接实现、先做探针里给出一个建议。若未自动触发，可以直接输入 `/sdlc-coding-process` 调用。
...
| 阶段 | 回答的问题 | 触发用语示例 | 技能 |
|---|---|---|---|
| 开局与约定 | 给 agent 立什么常驻规矩：文字要不要文风下限、对外文档要不要去 AI 味、长任务要不要放开自主执行 | 要给 agent 立什么规矩、怎么去 AI 味、发版说明太像 AI 写的、复盘太模板、agent 总中途停下来问、AGENTS.md 里写什么约定 | `sdlc-bootstrap` |
| 需求与规格 | 需求写到多细、按什么句式写、谁来评审，还是口头确认即可 | 怎么写 PRD、需求要写到多细、模糊想法怎么变成规格、需求评审怎么做 | `sdlc-requirements` |
| 设计 | 界面该用现成组件库、沿用已有规范、AI 直接生成，还是自建 token 体系 | 用哪个设计系统、要不要建 design token、UI 怎么做才不像模板 | `sdlc-design` |
| 编码 | 这次改动该走 TDD、SDD、直接实现还是先做探针 | 这次改动要不要先写测试、该走 TDD 还是 SDD、重构前要准备什么 | `sdlc-coding-process` |

其余阶段（测试、发布、运维）在路线图中。

## 工作方式

每个阶段对应两个文件：一份选型矩阵 `DECIDE.md`，以及一个只负责指向它的路由技能。技能的 description 填的是用户真实的中文问法，按平时的说法提问就能触发。
...
矩阵的每一行都写明适用条件、代价和反模式，因此 agent 直接给建议，不留一张让用户自己挑的清单。建议指向的执行工具多数是现成的社区技能，本库只登记何时使用、如何安装，不复制它们。

## 与技能聚合库的区别

技能聚合库按类型分目录，读者自行查找与挑选，回答的是"我已知道需要什么，去哪里找"。本库回答的是"我不知道该用什么"：读者不翻目录，提问之后由 agent 路由进对应矩阵。
...
技能每次被触发时会静默检查本库是否落后于远端，最多每 4 小时检查一次。落后时 agent 会带回一行提示，不会自动更新。
...
当前处于 M0（格式验证）：四个阶段（开局与约定、需求与规格、设计、编码）的决策入口已可使用，正在真实使用中验证矩阵格式与自动触发的准确度。代码只有两个 shell 脚本，以及 `site/` 下用于渲染站点的静态工程。
