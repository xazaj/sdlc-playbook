...
- **交付阶段**做减法。原文的说法是「AI 爱做加法，很少做减法」。删掉光晕、多余渐变和重复标签，把自定义控件换回系统原生控件；克制本身就是高级感的来源。
...
因此固化 prompt 里关于具体禁用项的那几条，是依据公开讨论中反复出现的共识特征补写的，不是原文转述；评审子代理的提问结构来自二手来源，未经原文核对。
...
````text
【AI 设计三阶段】会话级启用：直接粘贴，无需安装

本次会话中每次由你生成界面时，按三阶段走：

1. 发散：动手前先生成一串随机字母数字种子，从种子的不同片段派生至少三个
   候选方向的配色、字体搭配与版面节奏，三个方向必须互不雷同。方向表述成
   具体参照（「每一屏都像横版游戏的一帧」），不用形容词（「现代简洁」不可用）。
2. 收敛：我选定方向后，截图评审。你能开全新上下文的子代理就把评审交给它，
   只给截图不给代码；不能就先截图、隔一步再评，同样只看图。给 1 到 10 分
   与按影响排序的前三差距；达到约定分数线、或连续两轮不再改善即停。
3. 减法：收尾前删掉光晕与多余渐变、重复标签和无意义装饰边框；标准控件行为
   一律换回系统原生控件。禁用默认脸：蓝紫渐变，Inter 或 Roboto 唯一字体，
   居中英雄区加三栏卡片网格，emoji 当图标。

执行要求：先复述你理解的三个阶段与停止条件，确认后再生成。
````
...
付费墙后的第七项内容一旦可读，需要回来补齐并修订固化 prompt 中依据共识补写的那几条。
