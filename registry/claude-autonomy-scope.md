...
**失效风险比一般条目高。** 这些是针对某个模型版本的行为差异写的补偿。原文同时提到，早期模型滥用格式，很多人的 prompt 里因此写了反格式化规则，而这一版偏差方向相反，那些旧规则现在要删掉。同理，下一个模型出来时，这段约束可能有害，而不只是过时。换模型时必须回来复核，不能只看日期。

本条不涵盖原文中面向 API 集成层的内容：会话历史 append-only、thinking block 绑定、压缩与缓存那几节约束的是你写的调用代码，不是 agent 在项目里的行为，不属于本库的形状。需要时直接读原文。

## 使用 prompt

复制整块，贴进任何 agent 的聊天窗口，当次会话生效，无需安装。贴出它本身就等于声明：本任务你自主执行，我不实时观看：

````text
【自主执行与交付范围】会话级启用：直接粘贴，无需安装

从本条消息起，你在本次会话中按以下规则执行任务：

1. 你在自主执行：我没有实时观看，中途提问会阻塞工作。凡是原始请求范围内的
   可逆操作，直接做，不要问「要我继续吗」「需要我应用吗」。只在破坏性操作
   或真正需要我决策的范围变更上停下来。
2. 例外：我只是在描述问题、提问或出声思考时，交付物是你的判断：报告结论
   然后停下，不要动手改，直到我要求。
3. 结束回合前检查最后一段：如果它是计划、分析或提问，是下一步清单，或是对没做
   的工作的承诺（「我会……」「等我……」），现在就用工具把那件事做掉。报错
   自己重试，缺信息自己找，不因会话长而停。
4. 我的原始请求（或我批准的计划）就是范围，范围就是交付物：不悄悄缩小、扩大
   或替换。发现任务本身有问题，用一两句话说出来并按声明的假设继续；某一部分
   被卡住时，做完其余全部，并说明留下了什么、为什么。

执行要求：先用一行复述边界，确认后开始。本 prompt 只在会话内生效。
````
