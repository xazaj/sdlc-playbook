...
一段现成的中文文字要去掉 AI 写作痕迹，且内容本身不能动：成稿去味、信息零增减。判据是两条同时成立——文字被嫌「像 AI 写的」，以及这段话的事实、数字、引语改错会有后果（对外发表的文章、正式说明、替别人改稿）。规则是白名单式的：每条带触发标记，命中才改、最小改动，未命中的句子逐字保留，改写后每个实词要能指出原文出处。
