...
不适用：工程域文档（发版说明、复盘、PR/issue 回复、工单、技术文章——sepia 的域规则更对口，且两者触发面都覆盖「让文字不那么像 AI」，同一项目二选一，不许同装）；中文写作（本技能的模式表与删词表全是英文，对中文无效）；对话、代码注释、提交信息、内部笔记（常驻文风约束管的，任何去 slop 技能都不该碰）。
